...
def matrix2json(reaction_deficient,compound_deficient):
    ...
    将对应的数据库内数据进行预加载
    '''
    jsontext = {'metabolites':[],'reactions':[]}
    ...
    for ER in reaction_info:
        ...
        if flag:continue
        else:jsontext['reactions'].append({'id':ER[0],'metabolites':metabolites,'lower_bound':0.0,'upper_bound':999999.0,'ecnum':ecnum,'reaction_class':reaction_class,'energy':energy})
